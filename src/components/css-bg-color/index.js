...
class PGcssBgColor extends Component {
  ...
  render() {
    ...
    return (
      <div>
        <div className='my-4'>
          <p className='text-sm'>{__("This css property is depricated, please use", "team")} <strong>{__("Background Color", "team")}</strong> {__("instead", "team")}</p>
          <div className='relative h-10' style={placeholderStyle}>
            <div className='absolute w-full  h-full top-0 left-0 text-center' style={btnStyle} onClick={this.handleToggleClick}>
              <span className='w-full text-center left-0 top-1/2 -translate-y-1/2	 absolute'>{(val == undefined) ? __('Set Color', "team") : val}</span>
            </div>
          </div>
        </div>
